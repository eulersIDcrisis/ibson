...
class BsonPrimaryEncoderTests(unittest.TestCase):
    ...
    def test_encode_int64(self):
        ...
        actual = ibson.dumps(obj)
        expected = (
            b"\x14\x00\x00\x00\x12value\x00\x00\x00\x00\x00\x02\x00\x00\x00" b"\x00"
        )
        self.assertEqual(actual, expected)

        obj = dict(value=Int64(123))
        actual = ibson.dumps(obj)
        expected = b"\x14\x00\x00\x00\x12value\x00{\x00\x00\x00\x00\x00\x00\x00" b"\x00"
        self.assertEqual(actual, expected)
    ...
    def test_encode_utf8_string(self):
        # Test the full load.
        obj = dict(value="Ωhello")
        expected = (
            b"\x18\x00\x00\x00\x02value\x00\x08\x00\x00\x00\xce\xa9hello\x00" b"\x00"
        )
        actual = ibson.dumps(obj)
        self.assertEqual(actual, expected)
    ...
    def test_encode_bytes(self):
        obj = dict(value=b"\x04\x24")
        actual = ibson.dumps(obj)
        expected = b"\x13\x00\x00\x00\x05value\x00\x02\x00\x00\x00\x00\x04$\x00"
        self.assertEqual(actual, expected)

    def test_encode_uuid(self):
        gid = uuid.UUID("dc25ffb3-9a37-45db-a913-8b63a44fbbf4")
        obj = dict(value=gid)
        actual = ibson.dumps(obj)
        expected = (
            b"!\x00\x00\x00\x05value\x00\x10\x00\x00\x00\x04\xdc%\xff\xb3"
            b"\x9a7E\xdb\xa9\x13\x8bc\xa4O\xbb\xf4\x00"
        )
        self.assertEqual(actual, expected)


class BsonPrimaryDecoderTests(unittest.TestCase):
    ...
    def test_decode_utf8_string(self):
        ...
        obj = ibson.loads(data)
        self.assertEqual(obj, dict(value="Ωhello"))
    ...
    def test_decode_with_bson_types(self):
        expected = dict(
            short_int=Int32(432), long_int=Int64(123), uint_type=UInt64(124)
        )
